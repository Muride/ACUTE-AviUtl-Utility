...
class AviUtlGenerator:
    ...
    def detect_silence(self, file_path, threshold_db, min_silence_sec):
        ...
        self.print_log(f"[DEBUG] detect_silence コマンド実行: {' '.join(cmd)}")
        ...
        self.print_log(f"[DEBUG] detect_silence コマンド終了 (ReturnCode: {result.returncode})")
        ...
        try:
            with open(list_file_path, "w", encoding="utf-8") as f:
                for i, (start, end) in enumerate(keep_intervals):
                    ...
                    self.print_log(f"[DEBUG] chunk {i} 実行 (start:{start}, end:{end})")
                    res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='ignore', creationflags=subprocess.CREATE_NO_WINDOW)
                    self.print_log(f"[DEBUG] chunk {i} 完了 (ReturnCode: {res.returncode})")
                    if res.returncode != 0:
            ...
            self.print_log("[DEBUG] FFmpeg concat(結合) 実行開始")
            res_concat = subprocess.run(cmd_concat, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='utf-8', errors='ignore', creationflags=subprocess.CREATE_NO_WINDOW)
            self.print_log(f"[DEBUG] concat 完了 (ReturnCode: {res_concat.returncode})")
...
"""
    ...
    def transcribe_segment_based(self, file_path, base_file_path, model_size="medium", remove_filler=True, 
        ...
        # --- モデルの破棄によるC++自爆エラー(c0000409)を完全に防ぐための修正 ---
        ...
        # vad_filter=True を追加し、終端のノイズによるC++の自爆を完璧にガードします。
        # condition_on_previous_text=False で過去の文脈に引っ張られるエラーも防ぎます。
...
class App(ctk.CTk, TkinterDnD.DnDWrapper):
    def __init__(self):
        ...
        self.title("ACUTE v1.1.2 - AviUtl Cut Utility & Text Editor")
        ...
        # 6. 実行ボタン
        self.btn_run = ctk.CTkButton(self.frame_left, text="処理開始 (ACUTE v1.1.2)", height=60, font=("", 18, "bold"), command=self.run_process)
    ...
    def run_process(self):
        try:
            ...
            self.log("=== 処理開始 (ACUTE v1.1.2) ===")
            self.log(f"Whisper Model: {whisper_size}")
            if use_gemini:
                self.log(f"Gemini Model: {gemini_model}")
            
            self.log("[DEBUG] STEP 1: 動画の長さを取得します")
            base_duration = self.generator.get_duration(base_file)
            self.log(f"[DEBUG] STEP 1 完了: 長さ {base_duration}秒")

            self.log("[DEBUG] STEP 2: 無音解析(detect_silence)を開始します")
            keep_parts = self.generator.detect_silence(base_file, th, min_sil)
            self.log(f"[DEBUG] STEP 2 完了: 無音解析完了、{len(keep_parts)}件の区間を取得")

            self.log("[DEBUG] STEP 3: 余白と結合(apply_margin_and_gap)を計算します")
            ...
            self.log("[DEBUG] STEP 4: 一時WAVファイル生成(generate_cut_audio)を開始します")
            self.generator.generate_cut_audio(trans_file, final_keep_parts, temp_wav)
            self.log("[DEBUG] STEP 4 完了: 一時WAVファイル生成成功")
            
            self.log("[DEBUG] STEP 5: 文字起こし(transcribe_segment_based)を開始します")
